# Spanish (Latin America) Translation Guide
...
3. **Generate the Spanish illustration** using the "Illustration Translation" steps in `image-generation.md` — run `codex exec -i <original.png> "@imagegen edit…"` with the label-by-label translation mapping, preserving the exact layout, colours, and style
...
**Cost:** None — Codex `@imagegen` is billed to the ChatGPT plan, no per-image cost. Add each translated illustration to the running image count.
...
This guide is called from Phase 5.3 of `blog-writing-workflow.md`, immediately after the reviewer approves the English article (Step 5.2). The translation is included in the same publish commit as the English article.
