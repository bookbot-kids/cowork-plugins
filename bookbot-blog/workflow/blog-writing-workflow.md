...
- Images are generated via Codex `@imagegen` (`gpt-image-2`), billed to the ChatGPT plan with **no per-image cost**. After generation, report only the **count** of images produced — never a dollar figure. See Step 3.5.3.
...
Optional:
- **CATEGORY**: Which subcategory the article belongs in (if not provided, suggest one). One of: Dyslexia, EdTech, Science of Reading, Tips for Parents.
- **PUBLISH_DATE**: The date the article should go live (Australia/Melbourne time). If provided and in the future, the article is committed now but stays hidden on the live site until that date (Hugo excludes future-dated content; the daily rebuild surfaces it). If omitted, use today. See the front-matter `date` rules in `hugo-output-spec.md`.
...
All images are generated with Codex `@imagegen` per `image-generation.md`. Every image must be **themed to the article's category** (Dyslexia, EdTech, Science of Reading, Tips for Parents) and use **very descriptive prompts**.

#### Step 3.5.1: Generate Hero Image

1. Analyze the article's main theme, key visual concepts, and emotional tone
2. Construct a detailed, category-themed photographic hero prompt using the template in `image-generation.md`
3. Generate via Codex `@imagegen` (wide 16:9 landscape, photographic), attaching the best-fitting reference photo from `references/photos/` with `-i`
...
2. Construct a detailed, category-themed prompt tied to the surrounding article content using the illustration (or photo) template from `image-generation.md`
3. Attach the brand references via `-i` — `references/logo.png`, `references/reference-illustration.webp`, `references/yellow1.1.png` for illustrations; a `references/photos/*` photo for photographic inline — so Codex matches the Bookbot brand
4. Generate via Codex `@imagegen` (appropriate aspect ratio in words)
...
#### Step 3.5.3: Report Image Count

Codex `@imagegen` is billed to the ChatGPT plan — there is **no per-image cost**. After all images are generated, report only the count:

> **Images generated:** [N] via Codex `@imagegen` (gpt-image-2) — billed to the ChatGPT plan, no per-image API cost.

Images are reviewed as part of the Phase 5 review loop. If the reviewer requests changes, regenerate with updated prompts and update the running count.
...
**Image changes:**
1. Regenerate the specific image(s) with an updated prompt via Codex `@imagegen` (see `image-generation.md`)
...
4. For each inline illustration that contains text labels, follow the "Illustration Translation" section in `spanish-translation.md`:
   ...
   - Edit the original image via Codex `@imagegen` with the translation mapping (see `image-generation.md` "Illustration Translation")
...
The social media guide handles everything: deployment wait, image selection, hook generation, user review, and posting to all four platforms (Facebook, Instagram, Pinterest, LinkedIn).
...
All alt text: [list]
Images generated: [N] via Codex @imagegen (gpt-image-2) — no per-image cost (ChatGPT plan)
Regenerations: [number, if any]
...
| Images generated | — | [N] (Codex @imagegen) | — |
